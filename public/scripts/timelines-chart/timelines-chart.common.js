...
var svgTextFit = require('svg-text-fit');
var ColorLegend = require('d3-color-legend');
var d3Brush = require('d3-brush');

function _interopDefaultLegacy (e) { return e && typeof e === 'object' && 'default' in e ? e : { 'default': e }; }

var Kapsule__default = /*#__PURE__*/_interopDefaultLegacy(Kapsule);
var d3Tip__default = /*#__PURE__*/_interopDefaultLegacy(d3Tip);
var ColorLegend__default = /*#__PURE__*/_interopDefaultLegacy(ColorLegend);
...
var TimeOverview = Kapsule__default["default"]({
  props: {
    width: {
      "default": 300
    },
  ...
  init: function init(el, state) {
    state.xGrid = d3Axis.axisBottom().tickFormat('');
    state.xAxis = d3Axis.axisBottom().tickPadding(0);
    state.brush = d3Brush.brushX().handleSize(24).on('end', function (event) {
    ...
    state.svg = d3Selection.select(el).append('svg').attr('class', 'brusher');
...
var timelines = Kapsule__default["default"]({
  props: {
    data: {
      "default": [],
      onChange: function onChange(data, state) {
        parseData(data);
        state.zoomX = [d3Array.min(state.completeFlatData, function (d) {
          return d.timeRange[0];
        }), d3Array.max(state.completeFlatData, function (d) {
    ...
    minSegmentDuration: {},
    zColorScale: {
      "default": d3Scale.scaleSequential(d3ScaleChromatic.interpolateRdYlBu)
    },
    zQualitative: {
      "default": false,
      onChange: function onChange(discrete, state) {
        state.zColorScale = discrete ? d3Scale.scaleOrdinal([].concat(_toConsumableArray(d3ScaleChromatic.schemeCategory10), _toConsumableArray(d3ScaleChromatic.schemeSet3))) : d3Scale.scaleSequential(d3ScaleChromatic.interpolateRdYlBu); // alt: d3.interpolateInferno
      }
    },
    zDataLabel: {
      "default": '',
  ...
  methods: {
    ...
    getSvg: function getSvg(s) {
      return d3Selection.select(s.svg.node().parentNode).html();
    },
    ...
    sortChrono: function sortChrono(state, asc) {
      ...
      function buildIdx(accessFunction) {
        ...
        var _loop = function _loop(i, len) {
          ...
          idx[key] = [d3Array.min(itmList, function (d) {
            return d.timeRange[0];
          }), d3Array.max(itmList, function (d) {
            return d.timeRange[1];
          })];
        };
  ...
  init: function init(el, state, options) {
    var elem = d3Selection.select(el).attr('class', 'timelines-chart');
    state.svg = elem.append('svg');
    state.overviewAreaElem = elem.append('div'); // Initialize scales and axes

    state.yScale = d3Scale.scalePoint();
    state.grpScale = d3Scale.scaleOrdinal();
    state.xAxis = d3Axis.axisBottom();
    state.xGrid = d3Axis.axisTop();
    state.yAxis = d3Axis.axisRight();
    state.grpAxis = d3Axis.axisLeft();
    buildDomStructure();
    addTooltips();
    addZoomSelection();
    setEvents(); //
    ...
    function buildDomStructure() {
      ...
      state.groupGradId = svgUtils.gradient().colorScale(d3Scale.scaleLinear().domain([0, 1]).range(state.groupBkgGradient)).angle(-90)(state.svg.node()).id();
      ...
      state.colorLegend = ColorLegend__default["default"]()(state.svg.append('g').attr('class', 'legendG').node());
      ...
      function invertOrdinal(val, cmpFunc) {
        ...
        if (scRange.length === 2 && scDomain.length !== 2) {
          ...
          scRange = d3Array.range(scRange[0], scRange[1], (scRange[1] - scRange[0]) / scDomain.length);
        }
    ...
    function addTooltips() {
      state.groupTooltip = d3Tip__default["default"]().attr('class', 'chart-tooltip group-tooltip').direction('w').offset([0, 0]).html(function (event, d) {
      ...
      state.lineTooltip = d3Tip__default["default"]().attr('class', 'chart-tooltip line-tooltip').direction('e').offset([0, 0]).html(function (event, d) {
      ...
      state.segmentTooltip = d3Tip__default["default"]().attr('class', 'chart-tooltip segment-tooltip').direction('s').offset([5, 0]).html(function (event, d) {
        ...
        var dateFormat = (state.useUtc ? d3TimeFormat.utcFormat : d3TimeFormat.timeFormat)("".concat(state.timeFormat).concat(state.useUtc ? ' (UTC)' : ''));
    ...
    function addZoomSelection() {
      console.log(options);
      state.zoom = options.messages.zoom;

      var getPointerCoords = function getPointerCoords(event) {
        return d3Selection.pointer(event, state.graph.node());
      };

      state.graph.on('mousedown', function (event) {
        if (d3Selection.select(window).on('mousemove.zoomRect') != null) // Selection already active
          return;
        ...
        d3Selection.select(window).on('mousemove.zoomRect', function (event) {
          ...
          state.svg.dispatch('zoomScent', {
            detail: {
              zoomX: [startCoords[0], newCoords[0]].sort(d3Array.ascending).map(state.xScale.invert),
              zoomY: [startCoords[1], newCoords[1]].sort(d3Array.ascending).map(function (d) {
          ...
          d3Selection.select(window).on('mousemove.zoomRect', null).on('mouseup.zoomRect', null);
          d3Selection.select('body').classed('stat-noselect', false);
          rect.remove();
          ...
          var newDomainX = [startCoords[0], endCoords[0]].sort(d3Array.ascending).map(state.xScale.invert);
          var newDomainY = [startCoords[1], endCoords[1]].sort(d3Array.ascending).map(function (d) {
            return state.yScale.domain().indexOf(state.yScale.invert(d)) + (state.zoomY && state.zoomY[0] ? state.zoomY[0] : 0);
          });
      ...
      state.resetBtn = state.svg.append('text').attr('class', 'reset-zoom-btn').text(state.zoom.reset).style('text-anchor', 'end').on('mouseup', function () {
        state.svg.dispatch('resetZoom');
      }).on('mouseover', function () {
        d3Selection.select(this).style('opacity', 1);
      }).on('mouseout', function () {
        d3Selection.select(this).style('opacity', .6);
      });
    }
    ...
    function setEvents() {
      ...
      state.svg.on('resetZoom', function () {
        ...
        var newZoomX = state.enableOverview ? state.overviewArea.domainRange() : [d3Array.min(state.flatData, function (d) {
          return d.timeRange[0];
        }), d3Array.max(state.flatData, function (d) {
          return d.timeRange[1];
        })],
            newZoomY = [null, null];
  ...
  update: function update(state) {
    ...
    function setupDimensions() {
      ...
      state.graphH = d3Array.min([state.nLines * state.maxLineHeight, state.maxHeight - state.topMargin - state.bottomMargin]);
    ...
    function adjustXScale() {
      state.zoomX[0] = state.zoomX[0] || d3Array.min(state.flatData, function (d) {
        return d.timeRange[0];
      });
      state.zoomX[1] = state.zoomX[1] || d3Array.max(state.flatData, function (d) {
        return d.timeRange[1];
      });
      state.xScale = (state.useUtc ? d3Scale.scaleUtc : d3Scale.scaleTime)().domain(state.zoomX).range([0, state.graphW]).clamp(true);
    ...
    function renderAxises() {
      ...
      var minHeight = d3Array.min(state.grpScale.range(), function (d, i) {
    ...
    function renderGroups() {
      ...
      newGroups.append('title').text(state.zoom.title);
    ...
    function renderTimelines(maxElems) {
      ...
      newSegments.on('mouseover', function () {
        ...
        svgUtils.moveToFront()(this);
        var hoverEnlarge = state.lineHeight * hoverEnlargeRatio;
        d3Selection.select(this).transition().duration(70).attr('x', function (d) {
          return state.xScale(d.timeRange[0]) - hoverEnlarge / 2;
        }).attr('width', function (d) {
          return d3Array.max([1, state.xScale(d.timeRange[1]) - state.xScale(d.timeRange[0])]) + hoverEnlarge;
        }).attr('y', function (d) {
        ...
        d3Selection.select(this).transition().duration(250).attr('x', function (d) {
          return state.xScale(d.timeRange[0]);
        }).attr('width', function (d) {
          return d3Array.max([1, state.xScale(d.timeRange[1]) - state.xScale(d.timeRange[0])]);
        }).attr('y', function (d) {
