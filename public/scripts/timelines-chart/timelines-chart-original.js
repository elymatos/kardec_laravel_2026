// Version 2.11.5 timelines-chart - https://github.com/vasturiano/timelines-chart
(function (global, factory) {
  ...
  function bisector$2(f) {
    ...
    if (f.length !== 2) {
      compare1 = ascending$6;
      compare2 = (d, x) => ascending$6(f(d), x);
      delta = (d, x) => f(d) - x;
    } else {
      compare1 = f === ascending$6 || f === descending ? f : zero$2;
      compare2 = f;
      delta = f;
    }
  ...
  const ascendingBisect$1 = bisector$2(ascending$6);
  ...
  function axis(orient, scale) {
    ...
    function axis(context) {
      var values = tickValues == null ? (scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain()) : tickValues,
          ...
          position = (scale.bandwidth ? center : number$5)(scale.copy(), offset),
  ...
  function transformer$2() {
    ...
    function rescale() {
      ...
      if (clamp !== identity$3) clamp = clamper$1(domain[0], domain[n - 1]);
  ...
  function formatLocale$3(locale) {
    var group = locale.grouping === undefined || locale.thousands === undefined ? identity$2 : formatGroup$1(map$2.call(locale.grouping, Number), locale.thousands + ""),
        currencyPrefix = locale.currency === undefined ? "" : locale.currency[0] + "",
        currencySuffix = locale.currency === undefined ? "" : locale.currency[1] + "",
        decimal = locale.decimal === undefined ? "." : locale.decimal + "",
        numerals = locale.numerals === undefined ? identity$2 : formatNumerals$1(map$2.call(locale.numerals, String)),
  ...
  const durationMonth = durationDay$2 * 30;
  const durationYear = durationDay$2 * 365;

  var second = newInterval$2(function(date) {
  ...
  var minute = newInterval$2(function(date) {
  ...
  var hour = newInterval$2(function(date) {
  ...
  var day$2 = newInterval$2(
    date => date.setHours(0, 0, 0, 0),
    (date, step) => date.setDate(date.getDate() + step),
    (start, end) => (end - start - (end.getTimezoneOffset() - start.getTimezoneOffset()) * durationMinute$2) / durationDay$2,
    date => date.getDate() - 1
  );
  ...
  var month = newInterval$2(function(date) {
    date.setDate(1);
    date.setHours(0, 0, 0, 0);
  }, function(date, step) {
  ...
  var utcMinute = newInterval$2(function(date) {
  ...
  var utcHour = newInterval$2(function(date) {
  ...
  var utcMonth = newInterval$2(function(date) {
    date.setUTCDate(1);
    date.setUTCHours(0, 0, 0, 0);
  }, function(date, step) {
  ...
  function ticker(year, month, week, day, hour, minute) {
    ...
    function tickInterval(start, stop, count) {
      ...
      const i = bisector$2(([,, step]) => step).right(tickIntervals, target);
  ...
  const [utcTicks, utcTickInterval] = ticker(utcYear$2, utcMonth, utcSunday$2, utcDay$2, utcHour, utcMinute);
  const [timeTicks, timeTickInterval] = ticker(year$2, month, sunday$2, day$2, hour, minute);
  ...
  function time() {
    return initRange$1.apply(calendar(timeTicks, timeTickInterval, year$2, month, sunday$2, day$2, hour, minute, second, timeFormat).domain([new Date(2000, 0, 1), new Date(2000, 0, 2)]), arguments);
  }

  function utcTime() {
    return initRange$1.apply(calendar(utcTicks, utcTickInterval, utcYear$2, utcMonth, utcSunday$2, utcDay$2, utcHour, utcMinute, second, utcFormat$1).domain([Date.UTC(2000, 0, 1), Date.UTC(2000, 0, 2)]), arguments);
  }
  ...
  function sequential() {
    var scale = linearish$1(transformer$1()(identity$3));
  ...
  function d3Select(selector) {
    return typeof selector === "string"
        ...
        : new Selection$4([[selector]], root$3);
  }
  ...
  function onAdd$2(typename, value, capture) {
    var wrap = filterEvents.hasOwnProperty(typename.type) ? filterContextListener : contextListener$2;
  ...
  function d3Tip() {
    ...
    function initNode() {
      var div = select$2(document.createElement('div'));
      div
  ...
  cubehelixLong(cubehelix$2(-100, 0.75, 0.35), cubehelix$2(80, 1.50, 0.8));

  cubehelixLong(cubehelix$2(260, 0.75, 0.35), cubehelix$2(80, 1.50, 0.8));
  ...
  var gradient = index({
    ...
    init: function init(el, state) {
      ...
      state.gradient = select$1(el).append('linearGradient');
    },
  ...
  index({
    ...
    init: function init(el, state) {
      state.filter = select$1(el).append('defs').append('filter').attr('height', '130%');
  ...
  index({
    ...
    init: function init(el, state) {
      state.img = select$1(el).append('image');
    },
    update: function update(state) {
  ...
  index({
    ...
    init: function init(el, state, _ref) {
      ...
      state.svg = select$1(el).append('svg').attr('width', width).attr('height', height);
  ...
  function transition_attr(name, value) {
    var fullname = namespace$3(name), i = fullname === "transform" ? interpolateTransformSvg : interpolate;
...
console.log(_);
    ...
    init: function init(el, state, options) {
      ...
      function buildDomStructure() {
        ...
        state.groupGradId = gradient().colorScale(linear$2().domain([0, 1]).range(state.groupBkgGradient)).angle(-90)(state.svg.node()).id();
        ...
        function invertOrdinal(val, cmpFunc) {
          ...
          if (scRange.length === 2 && scDomain.length !== 2) {
            ...
            scRange = range$1(scRange[0], scRange[1], (scRange[1] - scRange[0]) / scDomain.length);
          }
        ...
        function addOverviewArea() {
          ...
          console.log(state.overviewArea.domainRange());
      ...
      function addTooltips() {
        ...
        state.segmentTooltip = d3Tip().attr('class', 'chart-tooltip segment-tooltip').direction('s').offset([5, 0]).html(function (event, d) {
          ...
          var dateFormat = (state.useUtc ? utcFormat$1 : timeFormat)("".concat(state.timeFormat).concat(state.useUtc ? ' (UTC)' : ''));
        ...
        state.graph.on('mousedown', function (event) {
          ...
          d3Select(window).on('mousemove.zoomRect', function (event) {
            ...
            state.svg.dispatch('zoomScent', {
              detail: {
                zoomX: [startCoords[0], newCoords[0]].sort(ascending$6).map(state.xScale.invert),
                zoomY: [startCoords[1], newCoords[1]].sort(ascending$6).map(function (d) {
            ...
            var newDomainX = [startCoords[0], endCoords[0]].sort(ascending$6).map(state.xScale.invert);
            var newDomainY = [startCoords[1], endCoords[1]].sort(ascending$6).map(function (d) {
              return state.yScale.domain().indexOf(state.yScale.invert(d)) + (state.zoomY && state.zoomY[0] ? state.zoomY[0] : 0);
            });
