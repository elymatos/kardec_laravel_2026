...
use App\Offline\Multimodal\audioPreprocess_Livia;
use App\Offline\Multimodal\videoPreprocess_Livia;
...
Artisan::command('webtool40_multimodal_video_process_doc {idDocument}', function ($idDocument) {
    $service = new videoPreprocess_Livia($idDocument);
    $service->process();
})->purpose('');

Artisan::command('webtool40_multimodal_audio_process_doc {idDocument}', function ($idDocument) {
    $service = new audioPreprocess_Livia($idDocument);
    $service->process();
})->purpose('');
